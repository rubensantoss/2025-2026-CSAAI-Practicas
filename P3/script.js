...
// Si vas a usar imágenes, instáncialas aquí. Por ahora dibujaremos vectores en el Canvas.
// const imgPlayer = new Image(); imgPlayer.src = 'img/caza.png';
...
let playerBullets = [];
let enemyBullets = [];
let explosions = []; // Almacenará {x, y, frames}
...
function update(deltaTime) {
    ...
    // 2. Disparo y Energía del jugador
    const now = Date.now();
    
    // Recarga automática progresiva (1 unidad cada 500ms)
    ...
    // Ráfaga limitada (200ms entre disparos)
    if (keys.Space && energy > 0 && now - lastShotTime > 200) {
        ...
        energy--;
        lastShotTime = now;
        lastEnergyRecharge = now; // Reinicia el timer de recarga al disparar
        if (sfxLaser.src) { sfxLaser.currentTime = 0; sfxLaser.play(); } // Reproduce sonido si existe
        updateUI();
    }

    // 3. Movimiento de balas (Jugador)
    ...
    // 4. Lógica de la Flota Alienígena
    ...
    if (hitWall) {
        alienDirection *= -1;
        for (let a of aliens) {
            if (a.active) a.y += 20; // Bajar una línea
            
            // Si llegan a la altura del jugador, perdemos
    ...
    // 5. Disparos Enemigos Ocasionales (~1 vez por segundo en general)
    // 60 frames por segundo -> 1/60 probabilidad aprox para toda la flota
    ...
    for (let i = playerBullets.length - 1; i >= 0; i--) {
        ...
        for (let a of aliens) {
            if (a.active && b.x < a.x + a.width && b.x + b.width > a.x && b.y < a.y + a.height && b.y + b.height > a.y) {
                ...
                // Añadir explosión (dura 15 frames)
...
function draw() {
    ...
    // Dibujar Jugador (Placeholder: Caza azul)
    ctx.fillStyle = '#00f';
    ctx.beginPath();
    ...
    // Dibujar Aliens (Placeholder: Cuadrados verdes)
    ctx.fillStyle = '#0f0';
    for (let a of aliens) {
        if (a.active) {
    ...
    // Dibujar Balas Jugador (Laser azul cyan)
    ctx.fillStyle = '#0ff';
    for (let b of playerBullets) {
        ctx.fillRect(b.x, b.y, b.width, b.height);
    }

    // Dibujar Balas Enemigas (Láser rojo)
    ctx.fillStyle = '#f00';
    ...
    for (let i = explosions.length - 1; i >= 0; i--) {
        ...
        if (exp.frames > 0) {
            // Reemplazar esto con ctx.drawImage si usas una imagen de explosión
